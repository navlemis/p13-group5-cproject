...
int main() {
    ...
    while (1) {
        ...
        if (strcmp(userCommand1, "OPEN") == 0) {
            /* If no filename was provided, fall back to the default DB_NAME */
            if (userCommand2[0] == '\0') {
                strncpy(userCommand2, DB_NAME, sizeof(userCommand2) - 1);
                userCommand2[sizeof(userCommand2) - 1] = '\0';
                printf("CMS: No filename provided. Using default file \"%s\".\n", userCommand2);
            }
